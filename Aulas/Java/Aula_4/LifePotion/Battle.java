...
public class Battle {
    public static void main(String[] args) {
        Char hero = new Char("Sir Gallahad", 14, 5, 80, 2);
        ...
        while (hero.isAlive() && monster.isAlive()) {
            if((hero.getLife() <= 2 || hero.getLife() <= hero.getMax_life()/4) && hero.getPotion() > 0){
                hero.heal();   
            }
            else{
                hero.attack(monster);   
            ...
            if (monster.isAlive()) {
                if((monster.getLife() <= 2 || monster.getLife() <= monster.getMax_life()/4) && monster.getPotion() > 0){
                    monster.heal();   
                }
                else{
                    monster.attack(hero);    
